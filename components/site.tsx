...
export function SectionLabel({ children, light = false }: { children: React.ReactNode; light?: boolean }) { return <p className={`font-mono text-xs uppercase tracking-[.16em] ${light ? "text-muted-light" : "text-cyan"}`}><motion.span className="mb-2 block h-px origin-left" style={{ background: light ? "#94A3B8" : "currentColor", opacity: 0.25 }} variants={{ hidden: { scaleX: 0 }, show: { scaleX: 1, transition: { duration: 0.6, ease: EASE } } }} />{children}</p>; }
...
export function Header() {
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      last = (last + e.key.toLowerCase()).slice(-2);
      if (last === "vk") { last = ""; window.dispatchEvent(new CustomEvent("vijay-secret", { detail: "vk-keys" })); }
    };
  ...
  const onLogoClick = () => {
    clicks.current += 1;
    if (clicks.current >= 3) { clicks.current = 0; window.dispatchEvent(new CustomEvent("vijay-secret", { detail: "logo-triple" })); }
    else {
      if (resetTimer.current) window.clearTimeout(resetTimer.current);
      resetTimer.current = window.setTimeout(() => { clicks.current = 0; }, 900);
    }
  };
  return <motion.header initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6 }} className={`sticky top-0 z-20 border-b text-light backdrop-blur transition-all duration-300 ${scrolled ? "border-dark-line bg-dark shadow-[0_1px_0_rgba(34,211,238,.15)]" : "border-dark-line bg-dark/95"}`}><div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4 sm:px-10 lg:px-16"><Link href="/" onClick={onLogoClick} className="flex items-center gap-3 font-mono text-base font-bold tracking-[-.03em]"><span>VK</span><span className="h-4 w-px bg-cyan" /><span className="text-[13px] font-normal tracking-[.01em] text-muted-dark">Vijay Kumaran</span></Link><nav className="hidden items-center gap-6 font-mono text-xs uppercase tracking-[.06em] desktop:flex"><Link href="/work" className="hover:text-cyan">Work</Link><Link href="/thinking" className="hover:text-cyan">Thinking</Link><Link href="/services" className="hover:text-cyan">Services</Link><Link href="/about#audit" className="hover:text-cyan">About</Link><Link href="/about#audit" className="group text-cyan hover:text-white">Book an audit <span aria-hidden className="inline-block transition-transform duration-300 group-hover:translate-x-1">↗</span></Link></nav><details className="relative desktop:hidden"><summary className="cursor-pointer list-none font-mono text-xs uppercase tracking-[.12em] text-cyan">Menu +</summary><nav className="absolute right-0 top-8 z-10 grid min-w-48 gap-4 border border-dark-line bg-dark p-5 font-mono text-xs uppercase tracking-[.12em] shadow-xl"><Link href="/work">Work</Link><Link href="/thinking">Thinking</Link><Link href="/services">Services</Link><Link href="/about#audit">About</Link><Link href="/about#audit" className="text-cyan">Book an audit ↗</Link></nav></details></div></motion.header>;
}
...
export function PageShell({ children }: { children: React.ReactNode }) { return <><Header /><main>{children}</main><Footer /><RevealToast /></>; }
