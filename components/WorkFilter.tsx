...
const BUCKETS = ["All", "AI Systems", "Business Systems", "Automation", "Websites"] as const;
...
export function WorkFilter({ projects }: { projects: Project[] }) {
  ...
  return <div>
    ...
    <MotionSection key={active} className="[&>div]:border-b [&>div]:border-light-line">
      {visible.map((project) => <MotionItem key={project.slug}><article id={project.slug} className="group grid gap-5 py-7 transition-colors hover:bg-ink/[.02] lg:grid-cols-[72px_1fr_1.1fr_90px] lg:items-start"><span className="font-mono text-xs text-muted-light">{project.index}</span><div><ProjectThumb slug={project.slug} /><ProjectMark slug={project.slug} /><h2 className="display mt-3 text-3xl transition-colors group-hover:text-blue sm:text-4xl">{project.name}</h2><p className="mt-2 text-sm text-muted-light">{project.category}</p></div><div><p className="text-base leading-relaxed text-muted-light">{project.description}</p><p className="mt-3 text-sm leading-relaxed">{project.metric}</p><p className="mt-3 font-mono text-[11px] leading-relaxed text-muted-light">{project.proof}</p></div><div className="lg:text-right"><Link href={project.href} className="font-mono text-sm tracking-[.02em] text-blue hover:underline">Read case study <AnimatedArrow /></Link></div></article></MotionItem>)}
    </MotionSection>
  </div>;
}
